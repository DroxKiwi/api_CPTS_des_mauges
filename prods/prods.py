
from datetime import datetime
from fastapi import APIRouter, Form, HTTPException, Depends
from pydantic import BaseModel
from typing import Optional
from typing_extensions import Annotated
from bdd.cpts import tprod, tprof
from auth import User, get_current_active_user

prods = APIRouter(
    prefix="/prods",
    tags=["prods"]
)

class prod(BaseModel):
    prod_id:Optional[int]
    prof_ids:Optional[str]
    name:Optional[str]
    img:Optional[str]
    actif:Optional[bool]

defaultImg = "data:image/png;base64,"

@prods.get("/all", tags=['prods'])
async def get_events(current_user: Annotated[User, Depends(get_current_active_user)]):
    try:
        data:tprod = tprod().readWhere(f"1 = 1")
        response = []
        for res in data:
            p = prod(
                prod_id=res.prod_id,
                prof_ids=res.prof_ids,
                name=res.name,
                img=res.img,
                actif=res.actif,
            )
            response.append(p)
        return response
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500,detail=[])

@prods.get("/byid/{prod_id}", tags=['prods'])
async def get_articles(current_user: Annotated[User, Depends(get_current_active_user)], prod_id:int):
    try:
        data:tprod = tprod().readId(str(prod_id))
        r = prod(
            prod_id=data.prod_id,
            prof_ids=data.prof_ids,
            name=data.name,
            img=data.img,
            actif=data.actif
        )
        return r
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500,detail=[])

@prods.post("/new", tags=['prods'])
async def create_event(current_user: Annotated[User, Depends(get_current_active_user)]):
    try:
        newProd = tprod()
        allProds = tprod().readWhere("1 = 1")
        ind = 0
        for a in allProds:
            ind += 1
        ind += 1
        newProd.prof_ids = ''
        newProd.name = f"Nouveau dossier {ind}"
        newProd.img = defaultImg
        newProd.actif = True
        oCnx = newProd.oCnx
        curs = oCnx.cursor()
        curs.execute(newProd.insert())
        oCnx.commit()
        curs.close()
        return {'result':'done'}
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])
    
@prods.post("/remove", tags=['prods'])
async def remove_event(current_user: Annotated[User, Depends(get_current_active_user)], prod_id: int = Form()):
    try:
        prodToDelete:tprod = tprod().readId(str(prod_id))
        oCnx = prodToDelete.oCnx
        curs = oCnx.cursor()
        curs.execute(prodToDelete.delete())
        oCnx.commit()
        curs.close()
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])
    
@prods.post("/update", tags=['prods'])
async def update_article(current_user: Annotated[User, Depends(get_current_active_user)], prod_id: int= Form(), prof_ids: str=Form(),  name: str= Form(), img: str= Form(), actif: bool= Form()):
    try:
        prodToUpdate:tprod = tprod().readId(str(prod_id))
        prodToUpdate.prof_ids = prof_ids
        prodToUpdate.name = name
        prodToUpdate.img = img
        prodToUpdate.actif = actif
        oCnx = prodToUpdate.oCnx
        curs = oCnx.cursor()
        curs.execute(prodToUpdate.update())
        oCnx.commit()
        curs.close()
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])
    
################################################################
# prof
################################################################

class prof(BaseModel):
    prof_id:Optional[int]
    name:Optional[str]
    subtitle:Optional[str]
    description:Optional[str]
    img:Optional[str]
    tecTimeInsert:Optional[datetime]
    actif:Optional[bool]

@prods.get("/prof/all", tags=['prods'])
async def get_events(current_user: Annotated[User, Depends(get_current_active_user)]):
    try:
        data:tprof = tprof().readWhere(f"1 = 1")
        response = []
        for res in data:
            p = prof(
                prof_id=res.prof_id,
                name=res.name,
                subtitle=res.subtitle,
                description=res.description,
                img=res.img,
                tecTimeInsert=res.tectimeinsert,
                actif=res.actif,
            )
            response.append(p)
        return response
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500,detail=[])

@prods.get("/prof/byid/{prof_id}", tags=['prods'])
async def get_articles(current_user: Annotated[User, Depends(get_current_active_user)], prof_id:int):
    try:
        data:tprof = tprof().readId(str(prof_id))
        r = prof(
            prof_id=data.prof_id,
            name=data.name,
            subtitle=data.subtitle,
            description=data.description,
            img=data.img,
            tecTimeInsert=data.tectimeinsert,
            actif=data.actif,
        )
        return r
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500,detail=[])

@prods.post("/prof/new", tags=['prods'])
async def create_event(current_user: Annotated[User, Depends(get_current_active_user)]):
    try:
        now = datetime.now()
        actualDate = now.strftime("%Y-%m-%d %H:%M:%S")
        newProf = tprof()
        allProfs = tprof().readWhere("1 = 1")
        ind = 0
        for a in allProfs:
            ind += 1
        ind += 1
        newProf.name = f"Nouveau document {ind}"
        newProf.subtitle = "Sous-titre"
        newProf.description = "Description"
        newProf.img = defaultImg
        newProf.tectimeinsert = actualDate
        newProf.actif = True
        oCnx = newProf.oCnx
        curs = oCnx.cursor()
        curs.execute(newProf.insert())
        oCnx.commit()
        curs.close()
        return {'result':'done'}
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])
    
@prods.post("/prof/remove", tags=['prods'])
async def remove_event(current_user: Annotated[User, Depends(get_current_active_user)], prof_id: int = Form()):
    try:
        profToDelete:tprof = tprof().readId(str(prof_id))
        oCnx = profToDelete.oCnx
        curs = oCnx.cursor()
        curs.execute(profToDelete.delete())
        oCnx.commit()
        curs.close()
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])
    
@prods.post("/prof/update", tags=['prods'])
async def update_article(current_user: Annotated[User, Depends(get_current_active_user)], prof_id: int= Form(), name: str= Form(), subtitle: str=Form(), description: str=Form(), img: str= Form(), actif: bool= Form()):
    try:
        profToUpdate:tprof = tprof().readId(str(prof_id))
        profToUpdate.name = name
        profToUpdate.subtitle = subtitle
        profToUpdate.description = description
        profToUpdate.img = img
        profToUpdate.actif = actif
        oCnx = profToUpdate.oCnx
        curs = oCnx.cursor()
        curs.execute(profToUpdate.update())
        oCnx.commit()
        curs.close()
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])
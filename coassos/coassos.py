
from datetime import datetime
import uuid
from fastapi import APIRouter, Form, HTTPException, Response, status, Depends
from pydantic import BaseModel, Field
from typing import Optional
import pymysql
from typing_extensions import Annotated
from bdd.cpts import tcoassos
from auth import User, get_current_active_user, get_password_hash, Token

coassos = APIRouter(
    prefix="/coassos",
    tags=["coassos"]
)

defaultImg = "data:image/png;base64,"

class coasso(BaseModel):
    coassos_id:Optional[int]
    img:Optional[str]
    redirect_url:Optional[str]

@coassos.get("/all", tags=['coassos'])
async def get_coassos():
    try:
        data:tcoassos = tcoassos().readWhere(f"1 = 1")
        response = []
        for res in data:
            r = coasso(
                coassos_id=res.coassos_id,
                img=res.img,
                redirect_url=res.redirect_url
            )
            response.append(r)
        return response
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500,detail=[])
    
@coassos.post("/new", tags=['coassos'])
async def create_article(current_user: Annotated[User, Depends(get_current_active_user)]):
    try:
        newCoassos = tcoassos()
        newCoassos.img = defaultImg
        newCoassos.redirect_url = "null"
        oCnx = newCoassos.oCnx
        curs = oCnx.cursor()
        curs.execute(newCoassos.insert())
        oCnx.commit()
        curs.close()
        return {'result':'done'}
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])
    
@coassos.post("/remove", tags=['coassos'])
async def remove_article(current_user: Annotated[User, Depends(get_current_active_user)], coassos_id: int = Form()):
    try:
        coassoToDelete:tcoassos = tcoassos().readId(str(coassos_id))
        oCnx = coassoToDelete.oCnx
        curs = oCnx.cursor()
        curs.execute(coassoToDelete.delete())
        oCnx.commit()
        curs.close()
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])

@coassos.post("/update", tags=['coassos'])
async def update_coasso(current_user: Annotated[User, Depends(get_current_active_user)], coassos_id: int= Form(), img: str= Form(), redirect_url: str= Form()):
    try:
        coassoToUpdate:tcoassos = tcoassos().readId(str(coassos_id))
        coassoToUpdate.img = img
        coassoToUpdate.redirect_url = redirect_url
        oCnx = coassoToUpdate.oCnx
        curs = oCnx.cursor()
        curs.execute(coassoToUpdate.update())
        oCnx.commit()
        curs.close()
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])

from datetime import datetime
import uuid
from fastapi import APIRouter, Form, HTTPException, Response, status, Depends
from pydantic import BaseModel, Field
from typing import Optional
import pymysql
from typing_extensions import Annotated
from bdd.cpts import tglobaldata
from auth import User, get_current_active_user, get_password_hash, Token

globaldatas = APIRouter(
    prefix="/globaldatas",
    tags=["globaldatas"]
)

defaultImg = "data:image/png;base64,"

class globaldata(BaseModel):
    globaldata_id:Optional[int]
    tel:Optional[str]
    adr:Optional[str]
    postalcode:Optional[str]
    facebook:Optional[str]
    linkedin:Optional[str]
    chiffrepsl:Optional[int]
    chiffrecom:Optional[int]
    chiffrehab:Optional[int]

@globaldatas.get("/all", tags=['globaldatas'])
async def get_globaldatas():
    try:
        data:tglobaldata = tglobaldata().readWhere(f"1 = 1")
        response = []
        for res in data:
            r = globaldata(
                globaldata_id=res.globaldata_id,
                tel=res.tel,
                adr=res.adr,
                postalcode=res.postalcode,
                facebook=res.facebook,
                linkedin=res.linkedin,
                chiffrepsl=res.chiffrepsl,
                chiffrecom=res.chiffrecom,
                chiffrehab=res.chiffrehab,
            )
            response.append(r)
        return response
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500,detail=[])

@globaldatas.post("/update", tags=['globaldatas'])
async def update_globaldata(current_user: Annotated[User, Depends(get_current_active_user)], globaldata_id: int= Form(), tel: str= Form(), adr: str= Form(), postalcode: str= Form(), facebook: str= Form(), linkedin: str= Form(), chiffrepsl: int= Form(), chiffrecom: str= Form(), chiffrehab: str= Form()):
    try:
        globaldataToUpdate:tglobaldata = tglobaldata().readId(str(globaldata_id))
        globaldataToUpdate.tel = tel
        globaldataToUpdate.adr = adr
        globaldataToUpdate.postalcode = postalcode
        globaldataToUpdate.facebook = facebook
        globaldataToUpdate.linkedin = linkedin
        globaldataToUpdate.chiffrepsl = chiffrepsl
        globaldataToUpdate.chiffrecom = chiffrecom
        globaldataToUpdate.chiffrehab = chiffrehab
        oCnx = globaldataToUpdate.oCnx
        curs = oCnx.cursor()
        curs.execute(globaldataToUpdate.update())
        oCnx.commit()
        curs.close()
    except Exception as e:
        print(e)
        raise HTTPException(status_code=500, detail=["Une erreur est survenue"])